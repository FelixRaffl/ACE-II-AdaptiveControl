...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(adaptive_dcmotor_M) == (NULL));
  while (runModel) {
    ...
    adaptive_dcmotor_step();

    /* Get model outputs here */

    /* Trigger External Mode event */
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(adaptive_dcmotor_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(adaptive_dcmotor_M, "stopping the model");
}
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(adaptive_dcmotor_M, 0);
  ...
  /* Initialize model */
  adaptive_dcmotor_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(adaptive_dcmotor_M->extModeInfo, &rtmGetTFinal
    (adaptive_dcmotor_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(adaptive_dcmotor_M, true);
    }
  }
