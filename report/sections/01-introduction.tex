...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[right=11mm of plant] (y) {$y$};

    \node[smallblock, above=15mm of io] (pp) {pole\\placement};
    \node[smallblock, above=15mm of plant] (rls) {RLS\\estimator};

    \draw[signal] (r) -- (sum);
    \draw[signal] (sum) -- node {$e$} (pi);
    \draw[signal] (pi) -- node {$u$} (io);
    \draw[signal] (io) -- (plant);
    \draw[signal] (plant) -- node {$y$} (y);
    \draw[signal] ($(plant.east)+(4mm,0)$) |- ++(0,-13mm) -| node[pos=0.95, left] {$-$} (sum.south);

    \draw[signal] ($(pi.east)+(4mm,0)$) |- (rls.west);
    \draw[signal] ($(plant.east)+(5mm,0)$) |- (rls.east);
    \draw[signal] (rls) -- node {$\hat a_0,\hat b_0$} (pp);
    \draw[signal] (pp) -- node {$d_0,d_1$} (pi);
