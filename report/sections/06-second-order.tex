...
The same conclusion was confirmed by the test rig in a practical way. The
second-order controller was tried on the hardware with
\(T=\SI{80}{ms}\). The closed loop followed the reference stably, but the
argument above implies that the parameter estimate cannot converge under that
sampling. A stable closed loop is therefore not evidence of successful
identification. The RST controller only needs certain combinations of the
parameters, not their individual values.
...
the controller. The effect was observed experimentally: the control error grew
to \(\SI{88}{\percent}\).

Integral action is also required. Pure pole placement fixes only the poles, not
the static gain, and therefore leaves a steady-state error. The measured value
...
\begin{center}
  ...
  \captionof{figure}{Validation run of the second-order adaptive RST controller with
